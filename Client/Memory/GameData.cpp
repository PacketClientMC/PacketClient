...
void GameData::retrieveClientInstance() {
	...
	g_Data.clientInstance = reinterpret_cast<ClientInstance*>(g_Data.slimMem->ReadPtr<uintptr_t*>(g_Data.gameModule->ptrBase + clientInstanceOffset, {0x0, 0x0, 0x48}));

	#ifdef _DEBUG
	if (g_Data.clientInstance == 0) throw std::exception("Client Instance is 0");
	#endif
}

bool GameData::canUseMoveKeys() {
	MinecraftGame* mc = g_Data.clientInstance->minecraftGame;
	if (moduleMgr != nullptr && moduleMgr->getModule<ClickGUIMod>()->isEnabled()) return true;
	else if (mc == nullptr) {
		return false;
	}
...
bool GameData::isKeyDown(int key) {
	...
	if (keyMapOffset == 0x0) {
		...
		if (sigOffset != 0x0) {
			int offset = *reinterpret_cast<int*>((sigOffset + 3));
			keyMapOffset = sigOffset - g_Data.gameModule->ptrBase + offset + 7;
...
void GameData::updateGameData(GameMode* gameMode) {
	...
	if (g_Data.localPlayer != nullptr && gameMode->player == g_Data.localPlayer) {  // GameMode::tick might also be run on the local server
		...
		if (g_Data.localPlayer != nullptr) {
			GuiData* guiData = g_Data.clientInstance->getGuiData();
