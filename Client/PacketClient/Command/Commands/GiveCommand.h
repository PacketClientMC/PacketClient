...
public:
	...
	virtual bool execute(std::vector<std::string>* args) override {
		...
		ItemStack* item = g_Data.getLocalPlayer()->getSelectedItem();
		if (args->size() > 4) {
			std::string tag;
			...
			if (tag.size() > 1 && tag.front() == MojangsonToken::COMPOUND_START.getSymbol() && tag.back() == MojangsonToken::COMPOUND_END.getSymbol()) {
				if (args->size() > 4) {
					...
					g_Data.getLocalPlayer()->getTransactionManager()->addInventoryAction(InventoryAction(0, nullptr, item, 507, 99999));
					g_Data.getLocalPlayer()->getTransactionManager()->addInventoryAction(InventoryAction(0, nullptr, nullptr, item, nullptr, 1, 507, 99999));
					g_Data.getLocalPlayer()->getTransactionManager()->addInventoryAction(InventoryAction(0, &desc, nullptr, item, nullptr, 1, 507, 99999));
				}
			}
			else {
