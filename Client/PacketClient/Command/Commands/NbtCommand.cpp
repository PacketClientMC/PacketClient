...
bool NbtCommand::execute(std::vector<std::string>* args) {
	...
	Level* pointingStruct = g_Data.getLocalPlayer()->level;
	BlockActor* blockActor = g_Data.getLocalPlayer()->region->getBlockEntity(pointingStruct->block);
	...
	if (args->at(1) == "read" || args->at(1) == "save") {
		...
		if (item->item) {
			...
			InventoryAction first(0, &desc, nullptr, item, nullptr, item->count, 507, 99999);
			InventoryAction second(supplies->selectedHotbarSlot, nullptr, &desc, nullptr, item, item->count);
