...
void ClickGui::renderPacketCategory(Category category, MC_Color categoryMC_Color) {
							...
							case ValueType::BOOL_T: {
								...
								// Checkbox
								{
									...
									if (setting->value->_bool) {
										DrawUtils::setMC_Color(28, 107, 201, 1);
	...
	// Draw Category Header
	{
		...
		/*{
			...
			if (extended) {
				...
				if (clickGUI->color.getSelectedValue() == 0) stringAids2 = "MC_Color: Rainbow";
				if (clickGUI->color.getSelectedValue() == 1) stringAids2 = "MC_Color: Astolfo";
				if (clickGUI->color.getSelectedValue() == 2) stringAids2 = "MC_Color: Wave";
				if (clickGUI->color.getSelectedValue() == 3) stringAids2 = "MC_Color: RainbowWave";
...
void ClickGui::renderVapeCategory(Category category) {
	...
	float currMC_Color[4];  // ArrayList colors
	currMC_Color[3] = rcolors[3];
	...
	if (ourWindow->isExtended) {
		...
		for (auto& mod : moduleList) {
			...
			auto arrayMC_Color = ColorUtil::rainbowColor(8, 1.F, 1.F, 1);
			if (clickGUI->color.getSelectedValue() == 0) arrayMC_Color = ColorUtil::rainbowColor(5, 1, 1, ModuleList.size() * moduleIndex * 2); // Rainbow MC_Colors
			if (clickGUI->color.getSelectedValue() == 1) arrayMC_Color = ColorUtil::astolfoRainbow(ModuleList.size() * moduleIndex / 2, 1000); // Astolfo MC_Colors
			if (clickGUI->color.getSelectedValue() == 2) arrayMC_Color = ColorUtil::waveColor(clickGUI->r1, clickGUI->g1, clickGUI->b1, clickGUI->r2, clickGUI->g2, clickGUI->b2, ModuleList.size() * moduleIndex * 3); // Wave
			if (clickGUI->color.getSelectedValue() == 3) arrayMC_Color = ColorUtil::RGBWave(clickGUI->r1, clickGUI->g1, clickGUI->b1, clickGUI->r2, clickGUI->g2, clickGUI->b2, ModuleList.size() * moduleIndex * 3); // Wave
			...
			if (allowRender)
			{
				DrawUtils::fillRectangleA(rectPos, mod->isEnabled() ? arrayMC_Color : MC_Color(50, 50, 50, 255));
							...
							case ValueType::FLOAT_T: {
								...
								// Slider
								{
									...
									// Visuals & Logic
									{
										...
										{
											rect.z = rect.x + value;
											DrawUtils::fillRectangle(rect, arrayMC_Color, (areWeFocused || setting->isDragging) ? 1.f : 1.f);
										}
	...
	// Draw Category Header
	{
		...
		// Draw component
		{
			...
			string textStr = categoryName;
			auto arrayMC_Color = ColorUtil::rainbowColor(8, 1.F, 1.F, 1);
			if (clickGUI->color.getSelectedValue() == 0) arrayMC_Color = ColorUtil::rainbowColor(5, 1, 1, ModuleList.size() * 1); // Rainbow MC_Colors
			if (clickGUI->color.getSelectedValue() == 1) arrayMC_Color = ColorUtil::astolfoRainbow(ModuleList.size() * 1 / 2, 1000); // Astolfo MC_Colors
			if (clickGUI->color.getSelectedValue() == 2) arrayMC_Color = ColorUtil::waveColor(clickGUI->r1, clickGUI->g1, clickGUI->b1, clickGUI->r2, clickGUI->g2, clickGUI->b2, ModuleList.size() * 1 * 3); // Wave
			if (clickGUI->color.getSelectedValue() == 3) arrayMC_Color = ColorUtil::RGBWave(clickGUI->r1, clickGUI->g1, clickGUI->b1, clickGUI->r2, clickGUI->g2, clickGUI->b2, ModuleList.size() * 1 * 3); // Wave
...
void ClickGui::renderAstolfoCategory(Category category, MC_Color categoryMC_Color, MC_Color categoryMC_Color2) {
	...
	if (ourWindow->isExtended) {
		...
		for (auto& mod : ModuleList) {
			...
			auto arrayMC_Color22 = ColorUtil::rainbowColor(8, 1.F, 1.F, 1);
			if (clickGUI->color.getSelectedValue() == 0) arrayMC_Color22 = ColorUtil::rainbowColor(5, 1, 0.7, ModuleList.size() * moduleIndex * 2); // Rainbow MC_Colors
			if (clickGUI->color.getSelectedValue() == 1) arrayMC_Color22 = ColorUtil::astolfoRainbow(ModuleList.size() * moduleIndex / 2, 1000); // Astolfo MC_Colors
			if (clickGUI->color.getSelectedValue() == 2) arrayMC_Color22 = ColorUtil::waveColor(clickGUI->r1 - 20, clickGUI->g1 - 20, clickGUI->b1 - 20, clickGUI->r2 - 20, clickGUI->g2 - 20, clickGUI->b2 - 20, ModuleList.size() * moduleIndex * 3); // Wave
			if (clickGUI->color.getSelectedValue() == 3) arrayMC_Color22 = ColorUtil::RGBWave(clickGUI->r1 - 20, clickGUI->g1 - 20, clickGUI->b1 - 20, clickGUI->r2 - 20, clickGUI->g2 - 20, clickGUI->b2 - 20, ModuleList.size() * moduleIndex * 3); // Wave

			if (clickGUI->categoryColors) arrayMC_Color = categoryMC_Color;
			if (clickGUI->categoryColors) arrayMC_Color22 = categoryMC_Color2;
			...
			{
				shared_ptr<ClickModule> clickMod = getClickModule(ourWindow, mod->getRawModuleName());
				if (!clickMod->isExtended) DrawUtils::fillRectangleA(rectPos, !mod->isEnabled() ? MC_Color(37, 37, 37) : arrayMC_Color);
							...
							case ValueType::BOOL_T: {
								...
								// Checkbox
								{
									if (setting->value->_bool) {
										...
										DrawUtils::fillRectangle(boxPos, arrayMC_Color, isFocused ? 1 : 0.8f);
									}
								}
								...
								// Slider
								{
									...
									// Visuals & Logic
									{
										...
										value *= endXlol;  // Value is now pixel diff between start of bar and end of progress
										{
											...
											DrawUtils::fillRectangle(rect, arrayMC_Color22, (areWeFocused || setting->isDragging) ? 1.f : 1.f);
										}
										{
											rect.z = rect.x + value;
											DrawUtils::fillRectangle(rect, arrayMC_Color, (areWeFocused || setting->isDragging) ? 1.f : 1.f);
	...
	DrawUtils::flush();
	{
...

		{
			...
			auto arrayMC_Color2 = ColorUtil::rainbowColor(8, 1.F, 1.F, 1);
			if (clickGUI->color.getSelectedValue() == 0) arrayMC_Color2 = ColorUtil::rainbowColor(5, 1, 1, ModuleList.size() * moduleIndex * 2); // Rainbow MC_Colors
			if (clickGUI->color.getSelectedValue() == 1) arrayMC_Color2 = ColorUtil::astolfoRainbow(ModuleList.size() * moduleIndex / 2, 1000); // Astolfo MC_Colors
			if (clickGUI->color.getSelectedValue() == 2) arrayMC_Color2 = ColorUtil::waveColor(clickGUI->r1, clickGUI->g1, clickGUI->b1, clickGUI->r2, clickGUI->g2, clickGUI->b2, ModuleList.size() * moduleIndex * 3); // Wave
			if (clickGUI->color.getSelectedValue() == 3) arrayMC_Color2 = ColorUtil::RGBWave(clickGUI->r1, clickGUI->g1, clickGUI->b1, clickGUI->r2, clickGUI->g2, clickGUI->b2, ModuleList.size() * moduleIndex * 3); // Wave
			if (clickGUI->categoryColors) arrayMC_Color2 = categoryMC_Color;
			...
			DrawUtils::drawRoundRectangle(rectTest, arrayMC_Color, false);
		}
	}
