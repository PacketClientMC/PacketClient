...
void ModuleManager::initModules() {
	{
		...
		moduleList.push_back(std::shared_ptr<IModule>(new FastSwim()));
		...
		moduleList.push_back(std::shared_ptr<IModule>(new IRC()));
	...
	// Enable modules first time injecting
...
void ModuleManager::onPlayerTick(Player* plr) {
	if (!isInitialized()) return;
