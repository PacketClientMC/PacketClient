...
void findPathEntity(Entity* currentEntity, bool isRegularEntity) {
...
int niga = 0;
void AutoPlay::onTick(GameMode* gm) {
...
void AutoPlay::onPlayerTick(Player* plr) {
	auto player = g_Data.getLocalPlayer();
...
void AutoPlay::onSendPacket(Packet* packet) {
	auto player = g_Data.getLocalPlayer();
	if (player == nullptr) return;
	targetList.clear();
	g_Data.forEachEntity(findPathEntity);
