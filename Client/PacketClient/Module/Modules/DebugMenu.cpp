...
void DebugMenu::onTick(GameMode* gm) {
    shouldHide = true;
}

void DebugMenu::onPostRender(MinecraftUIRenderContext* renderCtx) {
    ...
    string display = "Display: " + width + "x" + height;
    string multiplayer;
    ...
    string textStr = 
        "Minecraft 1.19.0 (" + (interfaceMod->versionStr + " / PacketClient)\n")
        + multiplayer + "\n"
        + fps + "\n\n"
        + position + "\n\n"
        + yaw + "\n"
