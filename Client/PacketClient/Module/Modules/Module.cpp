	...
	if (this->enabled != enabled) {
		...
		if (enabled) {
			...
			if (toggleSounds->isEnabled() && g_Data.getLocalPlayer() != nullptr && !((GameData::isKeyDown('L') && GameData::isKeyDown(VK_CONTROL)) || GameData::shouldTerminate())) {
				...
				case 0: // Click
					level->playSound("random.click", *player->getPos(), toggleSounds->volume, 0.6f); // 0.6
					break;
				case 1: // Piston
					level->playSound("tile.piston.out", *player->getPos(), toggleSounds->volume, 1.f);
					break;
				case 2: // Sigma
			...
			if (toggleSounds->isEnabled() && g_Data.getLocalPlayer() != nullptr && !((GameData::isKeyDown('L') && GameData::isKeyDown(VK_CONTROL)) || GameData::shouldTerminate())) {
				...
				switch (toggleSounds->sound.getSelectedValue()) {
				case 0: // Click
					level->playSound("random.click", *player->getPos(), toggleSounds->volume, 0.5f); // 0.6
					break;
				case 1: // Piston
					level->playSound("tile.piston.in", *player->getPos(), toggleSounds->volume, 1.f);
					break;
				case 2: // Sigma
