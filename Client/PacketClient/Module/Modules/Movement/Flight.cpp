...
void Flight::onTick(GameMode* gm) {
	auto player = g_Data.getLocalPlayer();
	if (player == nullptr) return;
	tick++;
	...
	if (mode.getSelectedValue() == 6) {
		if (g_Data.canUseMoveKeys()) {
			...
			if (TimeUtil::hasTimedElapsed(100, true)) {
				player->setPos(vec3_t(pos->x, pos->y - 1.f, pos->z));
				timerVariable = 5;
			}
			if (TimeUtil::hasTimedElapsed(50, false)) blinkMod->setEnabled(false);
		}
	}
}

void Flight::onPlayerTick(Player* plr) {
	auto player = g_Data.getLocalPlayer();
	if (player == nullptr) return;
}

void Flight::onMove(MoveInputHandler* input) {
	auto player = g_Data.getLocalPlayer();
	if (player == nullptr) return;
