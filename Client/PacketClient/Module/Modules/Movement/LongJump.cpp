...
void LongJump::onTick(GameMode* gm) {
	auto player = g_Data.getLocalPlayer();
	if (player == nullptr) return;
	tick++;
...
void LongJump::onMove(MoveInputHandler* input) {
    auto player = g_Data.getLocalPlayer();
    if (player == nullptr) return;

    GameSettingsInput* input2 = g_Data.getClientInstance()->getGameSettingsInput();
    ...
    case 2: // Hive
        ...
        if (player->velocity.squaredxzlen() > 0.01) {
            MovePlayerPacket p = MovePlayerPacket(g_Data.getLocalPlayer(), player->getPos()->add(vec3_t(moveVec.x / 2.1f, moveVec.y / 2, moveVec.z / 2.1f)));
