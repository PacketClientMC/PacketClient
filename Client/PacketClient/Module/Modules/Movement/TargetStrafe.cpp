...
void findEntityTSS(Entity* currentEntity, bool isRegularEntity) {
...
void TargetStrafe::onTick(GameMode* gm) {
...
void TargetStrafe::onPlayerTick(Player* plr) {
	GameSettingsInput* input = g_Data.getClientInstance()->getGameSettingsInput();
...
void TargetStrafe::onMove(MoveInputHandler* input) {
}
...
void TargetStrafe::onLevelRender() {
	...
	if (!targetList.empty()) {
		anim++;
		DrawUtils::setMC_Color(1, 1, 1, 1.f);
...
void TargetStrafe::onPostRender(MinecraftUIRenderContext* renderCtx) {
}

void TargetStrafe::onPreRender(MinecraftUIRenderContext* renderCtx) {
}