...
void findEntity_AH(Entity* currentEntity, bool isRegularEntity) {
...
void AutoHive::onTick(GameMode* gm) {
...
void AutoHive::onPlayerTick(Player* plr) {
	auto killaura = moduleMgr->getModule<Killaura>();
	auto player = g_Data.getLocalPlayer();
	if (player == nullptr) return;
...
void AutoHive::onSendPacket(Packet* packet) {
}
