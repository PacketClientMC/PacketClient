...
void Breaker::onPreRender(MinecraftUIRenderContext* renderCtx) {
...
void Breaker::onPlayerTick(Player* plr) {
	auto player = g_Data.getLocalPlayer();
	if (player == nullptr) return;
...
void Breaker::onSendPacket(Packet* packet) {
	...
	// Silent
	if (packet->isInstanceOf<MovePlayerPacket>() && rotations.getSelectedValue() == 1) {
		if (destroy || eat) {
