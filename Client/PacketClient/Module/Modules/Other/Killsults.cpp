...
using namespace std;
Killsults::Killsults() : IModule(0, Category::OTHER, "Insults people you kill") {
...
const char* Killsults::getModuleName() {
    name = string("Killsults ") + string(GRAY) + mode.GetEntry(mode.getSelectedValue()).GetName();
    return name.c_str();
}
...
void Killsults::onPlayerTick(Player* plr) {
