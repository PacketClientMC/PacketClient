...
void InvManager::onTick(GameMode* gm) {
...
vector<int> InvManager::findStackableItems() {
	...
	for (int i = 0; i < 36; i++) {
		ItemStack* itemStack = g_Data.getLocalPlayer()->getSupplies()->inventory->getItemStack(i);
		if (itemStack->item != nullptr) {
			if ((*itemStack->item)->getMaxStackSize(0) > itemStack->count) {
				for (int i2 = 0; i2 < 36; i2++) {
					if (i2 == i) continue;
					ItemStack* itemStack2 = g_Data.getLocalPlayer()->getSupplies()->inventory->getItemStack(i2);
...
vector<int> InvManager::findUselessItems() {
	...
	// Filter armor
	{
		...
		for (int i = 0; i < 4; i++) {
			ItemStack* itemsteck = g_Data.getLocalPlayer()->getArmor(i);
...
bool InvManager::stackIsUseful(ItemStack* itemStack) {
...
bool InvManager::isLastItem(Item* item) {
	vector<Item*> items;
	for (int i = 0; i < 36; i++) {
		ItemStack* stack = g_Data.getLocalPlayer()->getSupplies()->inventory->getItemStack(i);
