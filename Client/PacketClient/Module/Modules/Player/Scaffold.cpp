...
void Scaffold::onMove(MoveInputHandler* input) {
	auto player = g_Data.getLocalPlayer();
	if (player == nullptr) return;
...
void Scaffold::onPlayerTick(Player* plr) {
	GameSettingsInput* input = g_Data.getClientInstance()->getGameSettingsInput();
	auto player = g_Data.getLocalPlayer();
	if (player == nullptr) return;
...
void Scaffold::onPreRender(MinecraftUIRenderContext* renderCtx) {
	auto player = g_Data.getLocalPlayer();
	if (player == nullptr) return;
...
bool Scaffold::buildBlock(vec3_t blockBelow) {
	...
	Block* block = g_Data.getLocalPlayer()->region->getBlock(vec3_ti(blockBelow));
	...
	if (blockLegacy->material->isReplaceable) {
		...
		if (foundBlock) {
			g_Data.getGameMode()->buildBlock(&blok, i);
...
bool Scaffold::predictBlock(vec3_t blockBelow) {
	...
	for (const vec3_ti& offset : blocks) {
		...
		Block* block = g_Data.getLocalPlayer()->region->getBlock(currentBlock);
		GameSettingsInput* input = g_Data.getClientInstance()->getGameSettingsInput();
		BlockLegacy* blockLegacy = (block->blockLegacy);
		auto player = g_Data.getLocalPlayer();
...
bool Scaffold::isUsefulBlock(ItemStack* itemStack) {
...
bool Scaffold::findBlocks(ItemStack* itemStack) {
