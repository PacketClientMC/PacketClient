...
void ArrayList::onPostRender(MinecraftUIRenderContext* renderCtx) {
	...
	if (g_Data.canUseMoveKeys() && !clickGUI->hasOpenedGUI) {
		...
		for (std::set<IModuleContainer>::iterator mod = moduleList.begin(); mod != moduleList.end(); ++mod) {
			...
			// Drawing
			auto i = ColorUtil::interfaceColor(curIndex);
			if (opacity > 0) DrawUtils::fillRectangleA(rectPos, MC_Color(0, 0, 0, opacity));
			switch (mode.getSelectedValue()) {
			case 0:
				DrawUtils::fillRectangleA(leftRect, MC_Color(i));
				if (mod->enabled)DrawUtils::fillRectangleA(underline, MC_Color(i));
				DrawUtils::drawText(textPos, &textStr, MC_Color(i), textSize, 1.f, true);
				break;
			case 1:
				DrawUtils::fillRectangleA(leftRect, MC_Color(i));
				if (mod->enabled) DrawUtils::fillRectangleA(underline, MC_Color(i));
				if (index == 1) DrawUtils::fillRectangleA(topLine, MC_Color(i));
				DrawUtils::fillRectangleA(bar, MC_Color(i));
				DrawUtils::drawText(textPos, &textStr, MC_Color(i), textSize, 1.f, true);
				break;
			case 2:
				DrawUtils::fillRectangleA(leftRect, MC_Color(i));
			...
			case 4:
				...
				if (index == 1) DrawUtils::fillRectangleA(topLine3, MC_Color(i));
				DrawUtils::fillRectangleA(bar, MC_Color(i));
				DrawUtils::drawText(textPos, &textStr, MC_Color(i), textSize, 1.f, true);
				break;
			case 5: DrawUtils::drawText(textPos, &textStr, MC_Color(i), textSize, 1.f, true); break;
			}
		...
		index++; int curIndex = -index * interfaceMod->spacing;
		auto i = ColorUtil::interfaceColor(curIndex);
		if (mode.getSelectedValue() == 0 || mode.getSelectedValue() == 1) DrawUtils::fillRectangleA(underline, MC_Color(i));
		moduleList.clear();
	}
}