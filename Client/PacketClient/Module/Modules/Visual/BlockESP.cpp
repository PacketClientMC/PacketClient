...
void BlockESP::onPreRender(MinecraftUIRenderContext* renderCtx) {
		...
		for (int x = (int)pos->x - range; x < pos->x + range; x++) {
			for (int z = (int)pos->z - range; z < pos->z + range; z++) {
				for (int y = (int)pos->y - range; y < pos->y + range; y++) {
					...
					if (dRender) {
						DrawUtils::setMC_Color(0.2, 0.9, 1, 1);
						DrawUtils::drawBox(blockPos, angle, width, 1);
					} else if (eRender) {
						DrawUtils::setMC_Color(0, 0.7, 0, 1);
						DrawUtils::drawBox(blockPos, angle, width, 1);
					} else if (rRender) {
						DrawUtils::setMC_Color(0.8, 0, 0, 1);
						DrawUtils::drawBox(blockPos, angle, width, 1);
					} else if (gRender) {
						DrawUtils::setMC_Color(1, 0.7, 0, 1);
						DrawUtils::drawBox(blockPos, angle, width, 1);
					} else if (iRender) {
						DrawUtils::setMC_Color(1, 0.9, 0.8, 1);
						DrawUtils::drawBox(blockPos, angle, width, 1);
					}
				}
			}
