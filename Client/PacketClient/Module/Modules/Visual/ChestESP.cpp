...
void ChestESP::onPreRender(MinecraftUIRenderContext* renderCtx) {
	...
	if (g_Data.canUseMoveKeys()) {
		...
		for (auto iter = bufferedChestList.begin(); iter != bufferedChestList.end(); ++iter) {
			...
			DrawUtils::setMC_Color(1.f, 1.f, 1.f, storage);
			...
			if (storageID == 54) DrawUtils::setMC_Color(1.f, 1.f, 1.f, storage);       // Normal Chest
			if (storageID == 146) DrawUtils::setMC_Color(1.f, 1.f, 1.f, storage);      // Trapped Chest
			if (storageID == 130) DrawUtils::setMC_Color(0.56f, 0.f, 0.56f, storage);  // Ender Chest
...
/*void ChestESP::onPreRender(MinecraftUIRenderContext* renderCtx) {
	...
	for (auto iter = bufferedChestList.begin(); iter != bufferedChestList.end(); ++iter) {
		DrawUtils::setMC_Color(1.f, 0.3f, 0.3f, 0.6f);
		DrawUtils::drawBox((*iter)->lower, (*iter)->upper, (float)fmax(0.2f, 1 / (float)fmax(1, g_Data.getLocalPlayer()->eyePos0.dist((*iter)->lower))), true);  // Fancy math to give an illusion of good esp
	}
}*/