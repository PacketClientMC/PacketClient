...
ClickGUIMod::ClickGUIMod() : IModule(VK_INSERT, Category::VISUAL, "A GUI that displays every module") {
	...
	registerEnumSetting("MC_Color", &color, 2);
...
void ClickGUIMod::onPlayerTick(Player* plr) {
...
void ClickGUIMod::onPostRender(MinecraftUIRenderContext* renderCtx) {
	...
	if (g_Data.canUseMoveKeys()) g_Data.getClientInstance()->releaseMouse();
}

void ClickGUIMod::onPreRender(MinecraftUIRenderContext* renderCtx) {
