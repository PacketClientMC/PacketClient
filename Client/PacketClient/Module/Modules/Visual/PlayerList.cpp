...
void findPlayers(Entity* currentEntity, bool isRegularEntity) {
...
void PlayerList::onTick(GameMode* gm) {
	playerList.clear();
	g_Data.forEachEntity(findPlayers);
	//sort(playerList.begin(), playerList.end(), CompareTargetEnArray());
}

void PlayerList::onPostRender(MinecraftUIRenderContext* renderCtx) {
	...
	if (g_Data.canUseMoveKeys() || !clickGUI->hasOpenedGUI) {
		...
		case 1:
			...
			DrawUtils::fillRectangleA(headerPos2, i);
			break;
		}
	}
}
