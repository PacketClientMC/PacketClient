...
void SessionInfo::onTick(GameMode* gm) {
...
void SessionInfo::onPostRender(MinecraftUIRenderContext* renderCtx) {
	...
	if (g_Data.canUseMoveKeys() && !clickGUI->hasOpenedGUI) {
		...
		case 0: // Packet
			...
			if (outline) DrawUtils::drawRoundRectangle(rectPos, i, false);
		...
		case 1: // Tenacity
			...
			if (outline) DrawUtils::drawRoundRectangle(rectPos, !clickGUI->hasOpenedGUI ? i : MC_Color(255, 255, 255), true);
			DrawUtils::fillRectangleA(linePos, MC_Color(255, 255, 255, 255));
			break;
		}
	}
