...
void findPlayers_TargetHUD(Entity* currentEntity, bool isRegularEntity) {
...
void TargetHUD::onPostRender(MinecraftUIRenderContext* renderCtx) {
			...
			for (auto& i : targetList) {
				if (mode.getSelectedValue() == 0) {
					...
					if (i->getEntityTypeId() == 319 && showItems) {
						...
						auto* player = reinterpret_cast<Player*>(targetList[0]);
						for (int t = 0; t < 4; t++) {
