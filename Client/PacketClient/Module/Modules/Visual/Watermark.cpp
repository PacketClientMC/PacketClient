...
void Watermark::onPostRender(MinecraftUIRenderContext* renderCtx) {
	...
	if (g_Data.canUseMoveKeys() && !clickGUI->hasOpenedGUI) {
		...
		//DrawUtils::drawRounderRectangle(vec4_t(positionX, positionY, positionX + 15, positionY + 2.F), interfaceMC_Color);
		...
		case 0:
			...
			else DrawUtils::drawText(vec2_t(simpleRect.x, simpleRect.y), &clientName, i, scale, 1, true);
			break;
		case 1: // New
			DrawUtils::fillRoundRectangle(newRect, MC_Color(0, 0, 0, opacity), false);
			DrawUtils::fillRectangleA(newRectTop, i);
			DrawUtils::drawText(vec2_t(newRect.x + (textPadding * 3), newRect.y + (textPadding * 3)), &newText, i, scale, 1, true);
			break;
		case 2: // Weird
