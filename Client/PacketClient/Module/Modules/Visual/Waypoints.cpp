...
using namespace std;
Waypoints::Waypoints() : IModule(0, Category::VISUAL, "Shows holograms for defined coordinates") {
	registerBoolSetting("Interdimensional", &interdimensional, interdimensional);
	registerBoolSetting("Coordinates", &showCoordinates, showCoordinates);
...
void Waypoints::onPreRender(MinecraftUIRenderContext* renderCtx) {
	...
	for (auto it = waypoints->begin(); it != waypoints->end(); it++) {
		...
		if (useFloatingPoint) {
			ostringstream out;
			out.precision(2);
			out << it->first << " (" << fixed << dist << " m";
			txt = out.str();
		} else {
			txt = it->first + " (" + to_string(int(dist)) + "m";
		}
		...
		if (fadeOutAtDistance && dist > 15) {
			...
			if (dist > 30) {
				float neededDiff = lerp(40, 15, min((dist - 30) / 300, 1.f));
				float minAlpha = lerp(0.6f, 0.3f, min((dist - 30) / 50, 1.f));
				if (diff < neededDiff)
					alpha = 1.f;
		...
		if (textPos.x != -1) {
			...
			if (this->showCoordinates) {
				ostringstream out;
				out.precision(1);
				out << "(" << fixed << pos.x << ", " << pos.y << ", " << pos.z << ")";
				coordText = out.str();
				textWidth = max(textWidth, DrawUtils::getTextWidth(&coordText, size * 0.75f) + 0.5f);
			}
