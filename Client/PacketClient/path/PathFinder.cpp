...
PathFinder::PathFinder(vec3_ti start, BlockSource* reg, std::shared_ptr<PathGoal> goal) : startPos(start), region(reg), goal(goal) {
}
...
__forceinline bool isDangerous(const vec3_ti& pos, BlockSource* reg, bool allowWater) {
...
__forceinline bool isDangerousPlayer(const vec3_ti& pos, BlockSource* reg, bool allowWater = false) {
	return isDangerous(pos, reg, allowWater) || isDangerous(pos.add(0, 1, 0), reg, allowWater);
}

__forceinline bool canStandOn(const vec3_ti& pos, BlockSource* reg, bool inWater = false) {
...
__forceinline bool isObstructed(const vec3_ti& pos, BlockSource* reg, bool allowWater = false) {
	auto block = reg->getBlock(pos);
...
__forceinline bool isObstructedPlayer(const vec3_ti& pos, BlockSource* reg, bool allowWater = false) {
		...
		for (int z = -1; z <= 1; z++) {
			...
			if (moduleMgr->getModule<AutoPlay>()->server.getSelectedValue() != 2) {
				if (!canStandOn(newPos.add(0, -1, 0), reg, isInWater)) {
					...
					if (isObstructedPlayer(newPos, reg)) {// landing zone
						...
						edges.emplace_back(startNodeRef, findNode(allNodes, jumpPos), PARKOUR_JUMP1_TIME, PathSegmentType::PARKOUR_JUMP_SINGLE);
						continue;
					}
					...
					if (!canStandOn(newPos.add(0, -1, 0), reg)) { // we can't stand on parkour jump landing zone, move it down and walk there?
						for (int dropLength = 1; dropLength <= 3; dropLength++) {
							...
							edges.emplace_back(startNodeRef, findNode(allNodes, dropPos), time, PathSegmentType::WALK);
							goto tryLargerParkourJump;
						}
						goto tryLargerParkourJump;
					}
					...
					if (canJump) {
						edges.emplace_back(startNodeRef, findNode(allNodes, newPos), PARKOUR_JUMP1_TIME, PathSegmentType::PARKOUR_JUMP_SINGLE);
						continue; // we don't need to try a larger jump, we could just walk there
					}
				tryLargerParkourJump:
					// Gives invalid pos warning thingy but I'm too lazy to fix rn

					std::vector<vec3_ti> stuff;
					...
					for (int jumpLength = 2; jumpLength <= 3; jumpLength++) {
						...
						if (canStandOn(stuff.at(jumpLength + 1).sub(0, 1, 0), reg) && !isObstructedPlayer(stuff.at(jumpLength + 1), reg)) {  // Can we actually land here
							edges.emplace_back(startNodeRef, findNode(allNodes, stuff.at(jumpLength + 1)), (jumpLength == 2) ? 0.7f : 1.f, PathSegmentType::PARKOUR_JUMP_SINGLE);
						}
					bad:
						continue;
					}
			...
			if (isInWater) {
				...
				if (block->material->isLiquid) {
					...
					if (!flow.iszero()) {
						auto tangent = newPos.sub(startNode.pos).toVector3();
						tangent.y = 0;
...
Path PathFinder::findPath() {
	...
	for (auto& nodeDesc : allNodes) {
		...
		auto dist = (int)roundf(node.pos.toVector3().dist(startPos.toVector3()));
