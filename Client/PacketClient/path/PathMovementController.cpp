...
void PathMovementController::step(LocalPlayer* player, MoveInputHandler* movementHandler) {
	...
	auto start = startBpos.toVector3().add(0.5f, 0, 0.5f);
	auto endBpos = curSeg.getEnd();
	auto end = endBpos.toVector3().add(0.5f, 0, 0.5f);

	auto nextSegEnd = vec3_t();

	bool hasNextSeg = this->stateInfo.currentPathSegment < this->currentPath->getNumSegments() - 1;
	if (hasNextSeg)
		nextSegEnd = this->currentPath->getSegment(this->stateInfo.currentPathSegment + 1).getEnd().toVector3().add(0.5f, 0, 0.5f);
	...
	case JUMP: {
		if (player->onGround) {
			...
			if (moduleMgr->getModule<AutoPlay>()->server.getSelectedValue() == 2) {
				...
				if (posToJumpTarg < maxJumpDist && posToJumpTarg > 0) {
					// jump
					g_Data.getLocalPlayer()->velocity.y += 0.4;
					goto WALK;
				}
			}
		}
	...
	case DROP: {
		...
		if (moduleMgr->getModule<AutoPlay>()->server.getSelectedValue() == 2) {
			...
			if (fabsf(pPos.y - end.y) < (inWater ? 0.2f : 0.1f) && pPos.sub(end).magnitudexz() < 0.5f && player->velocity.y > -0.1f) {// Check for end condition
				...
				dComp = 3;
				g_Data.getLocalPlayer()->velocity.y -= 0.5;
				goto WALK;
			}
		} else if (player->onGround || inWater) {
			dComp = 1;
			if (fabsf(pPos.y - end.y) < (inWater ? 0.2f : 0.1f) && pPos.sub(end).magnitudexz() < 0.5f && player->velocity.y > -0.1f) {// Check for end condition
				this->stateInfo.nextSegment();
				break;
