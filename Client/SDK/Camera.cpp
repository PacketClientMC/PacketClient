...
void Camera::getEulerAngles(vec3_t* vec) {
	using cam_getEuler_t = void(__fastcall*)(Camera*, vec3_t*);
...
void Camera::getPlayerRotation(vec2_t* angOut) {
	vec3_t angles;
...
void Camera::getForwardVector(vec3_t* forward) {
	vec3_t eulerAngles;
