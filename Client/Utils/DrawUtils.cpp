...
void initializeSigs() {
	...
	meshHelper_renderImm = reinterpret_cast<meshHelper_renderImm_t>(FindSignature("40 53 56 57 48 81 ec ? ? ? ? 49 8b f0 48 8b da 48 8b f9 80 ba"));
...
void DrawUtils::setCtx(MinecraftUIRenderContext* ctx, GuiData* gui) {
	...
	tesselator = *reinterpret_cast<Tessellator**>(screenContext2d + 0xB8);
...
void DrawUtils::setMC_Color(float r, float g, float b, float a) {
	colorHolder[0] = r;
	colorHolder[1] = g;
	colorHolder[2] = b;
	colorHolder[3] = a;
...
void DrawUtils::drawImage(std::string filePath, vec2_t& imagePos, vec2_t& imageDimension, vec2_t& idk, vec2_t& idk2, MC_Color flushMC_Color) {
	...
	if (texturePtr != nullptr) {
		...
		renderCtx->flushImages(flushMC_Color, flushImageAddr, flushString);
		delete texturePtr;
	}
}
...
void DrawUtils::draw2D(Entity* ent, float lineWidth) {
	...
	corners[4] = vec3_t(base.x + ent->width / 2.f * -sin(ofs), ent->aabb.lower.y, base.z + ent->width / 1.5f * cos(ofs));
	...
	if (refdef->OWorldToScreen(origin, corners[0], corners2d[0], fov, screenSize) &&
		...
		drawLine(vec2_t(corners2d[4].x, corners2d[4].y - length), corners2d[4], lineWidth);
		drawLine(vec2_t(corners2d[4].x - length - 4, corners2d[4].y), corners2d[4], lineWidth);
...
void DrawUtils::drawBetter2D(Entity* ent, float lineWidth) {
	float ofs = (g_Data.getLocalPlayer()->yaw + 90.f) * (PI / 180);
...
void DrawUtils::drawItem(ItemStack* item, vec2_t itemPos, float opacity, float scale, bool isEnchanted) {
	__int64 scnCtx = reinterpret_cast<__int64*>(renderCtx)[2];
	auto* screenCtx = reinterpret_cast<ScreenContext*>(scnCtx);
	BaseActorRenderContext baseActorRenderCtx(screenCtx, g_Data.getClientInstance(), g_Data.getClientInstance()->minecraftGame);
...
void DrawUtils::fillRectangle(vec4_t pos, const MC_Color col, float alpha) {
	DrawUtils::setMC_Color(col.r, col.g, col.b, alpha);
	DrawUtils::drawQuad({ pos.x, pos.w }, { pos.z, pos.w }, { pos.z, pos.y }, { pos.x, pos.y });
}

void DrawUtils::drawButton(vec4_t pos, vec2_t textPos, std::string* textStr, const MC_Color hoverMC_Color, MC_Color color) {
	...
	DrawUtils::drawRoundRectangle(vec4_t(pos.x, pos.y, pos.x + buttonLen, pos.y), pos.contains(&mousePos) ? hoverMC_Color : color, false);
}

void DrawUtils::fillRectangleA(vec4_t pos, MC_Color col) {
	DrawUtils::setMC_Color(col.r, col.g, col.b, col.a);
	DrawUtils::drawQuad({ pos.x, pos.w }, { pos.z, pos.w }, { pos.z, pos.y }, { pos.x, pos.y });
}
