...
void TargetUtil::init(LocalPlayer** cl) {
	localPlayer = cl;
}

bool TargetUtil::isValidTarget(Entity* ent) {
	...
	if (teams->isEnabled() && ent->getEntityTypeId() == 316) {
		...
		if (teams->isMC_ColorCheckEnabled()) {
			...
			if (TargetUtilName.length() > 2 && localName.length() > 2) {
				if (TargetUtilName.at(2) == localName.at(2) && ent->isPlayer()) {
					if (esp->isEnabled()) {
						DrawUtils::setMC_Color(0.1f, 0.9f, 0.1f, (float)fmax(0.1f, (float)fmin(1.f, 15 / (ent->damageTime + 1))));
