...
bool Utils::getShouldLocalPlayerBeImmobile() {
	return g_Hooks.isPlayerImmobile;
}
...
void Utils::makeFolderInPacket(std::string file) {
	std::string path = getenv("APPDATA") + std::string("\\..\\Local\\Packages\\Microsoft.MinecraftUWP_8wekyb3d8bbwe\\RoamingState\\PacketClient\\") + std::string(file);
	// Creating File
	if (mkdir(path.c_str()) == -1)
		return;
	else
...
void Utils::makeFolderInAssets(std::string file) {
	std::string path = getenv("APPDATA") + std::string("\\..\\Local\\Packages\\Microsoft.MinecraftUWP_8wekyb3d8bbwe\\RoamingState\\PacketClient\\Assets\\") + std::string(file);
	// Creating File
	if (mkdir(path.c_str()) == -1)
		return;
	else
